...
beforeEach(() => {
  ...
  process.env.TRIGGER_STATE_FILE = path.join(dir, "triggers.json");
});

const at = (condition: Condition, basePrice = 226) => ({ condition, basePrice });

describe("when a condition holds", () => {
  it("fires a 2.5% rise at the target and above, not below it", () => {
...
describe("what can be set", () => {
  it("refuses a price level that is already passed", () => {
  ...
  it("says the whole trigger in one sentence", () => {
    expect(
      describeTrigger({
        symbol: "NVDA",
...
describe("keeping triggers", () => {
  ...
  it("starts a trigger active and lists it for its owner only", () => {
    const t = addTrigger(base());
    expect(t.status).to.equal("active");
    expect(listTriggers("owner-a")).to.have.length(1);
    expect(listTriggers("owner-b")).to.have.length(0);
  });

  it("lets a trigger be claimed for firing once", () => {
    const t = addTrigger(base());
    expect(transition(t.id, "active", "firing")).to.not.equal(null);
    // A second pass finds it already claimed.
    expect(transition(t.id, "active", "firing")).to.equal(null);
    expect(activeTriggers()).to.have.length(0);
  });

  it("cannot cancel a trigger that has fired", () => {
    const t = addTrigger(base());
    transition(t.id, "active", "firing");
    transition(t.id, "firing", "fired", { result: "Bought." });
    expect(cancelTrigger("owner-a", t.id)).to.equal(null);
    expect(listTriggers("owner-a")[0].status).to.equal("fired");
  });

  it("cancels only the owner's own trigger", () => {
    const t = addTrigger(base());
    expect(cancelTrigger("owner-b", t.id)).to.equal(null);
    expect(cancelTrigger("owner-a", t.id)?.status).to.equal("cancelled");
    expect(activeTriggers()).to.have.length(0);
  });
});
